...
import { readScoreFile, mergeScores, mergeScoresOnDate } from "./symptomScores";
...
function getUniqueValues(patients, field) {
  const vals = [...new Set(patients.map(p => p[field]).filter(Boolean))].sort();
  return vals;
}
...
export default function App() {
  ...
  const [timelineDate, setTimelineDate] = useState(null);   // selected date from slider
  const [timelineEnv, setTimelineEnv]   = useState(null);   // env at selected date
  ...
  async function handleScoreFile(e) {
    ...
    try {
      ...
      setScoreMap(map);
      // Re-merge if demographics already loaded
  ...
  // Compute age/comorbidity bounds from data (no longer needed for sliders, kept for info)
  const ages = useMemo(() => patients.map(p => p.age).filter(a => a > 0), [patients]);
    ...
    <div style={{ fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", maxWidth: 900, margin: "0 auto", padding: 16, background: "#f8fafc" }}>
      ...
      <div style={{ background: "linear-gradient(135deg, #1e3a5f, #2d5a8e)", color: "#fff", padding: "16px 20px", borderRadius: 10, marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 12, opacity: 0.85, marginTop: 4 }}>
          Real-time environmental overlay | {csvLoaded ? "CSV loaded" : "Demo data"} | {patients.length} patients
        </div>
      </div>
      ...
      <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 8, padding: 12, marginBottom: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap", marginBottom: 8 }}>
          <label style={{ fontSize: 13, fontWeight: 600, color: "#374151", minWidth: 110 }}>Demographics CSV:</label>
          <input type="file" accept=".csv" onChange={handleCSV} style={{ fontSize: 12 }} />
          <span style={{ fontSize: 11, color: "#9ca3af" }}>
            {csvLoaded ? `${patients.length} patients loaded` : "id, lat, lng, comorbidities, dob..."}
          </span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          <label style={{ fontSize: 13, fontWeight: 600, color: "#374151", minWidth: 110 }}>Symptom Scores:</label>
          <input type="file" accept=".csv" onChange={handleScoreFile} style={{ fontSize: 12 }} />
          <span style={{ fontSize: 11, color: scoreMap ? "#059669" : "#9ca3af" }}>
            {scoreMap
              ? `${scoreMap.size} scores loaded | ${patients.filter(p => p.scoreSource === "actual").length} matched`
              : "patient_id, responded_date, symptoms_score"}
          </span>
        </div>
      </div>
      ...
      {/* Environment Panel — updates with timeline when scrubbing */}
